...
import org.edgexfoundry.protocol.DeviceDiscovery;
import org.edgexfoundry.protocol.ObjectTransform;
import org.edgexfoundry.protocol.ProtocolDriver;
...
public class ProtocolHandler {
	...
	private final static EdgeXLogger logger = EdgeXLoggerFactory.getEdgeXLogger(ProtocolHandler.class);
	...
	@Value("${protocol.device.init:#{null}}")
	private String protocolInit;
	@Value("${protocol.device.init.args:#{null}}")
	private String protocolInitArgs;
	
	@Value("${protocol.device.remove:#{null}}")
	private String protocolRemove;
	@Value("${protocol.device.remove.args:#{null}}")
	private String protocolRemoveArgs;
	...
	public void initializeDevice(Device device) {
		if(protocolInit != null && commandExists(device, protocolInit))
			executeCommand(device, protocolInit, protocolInitArgs);
		logger.info("Initialized Device: " + device.getName());
	}

	public void disconnectDevice(Device device) {
		if (protocolRemove != null && commandExists(device, protocolRemove))
			executeCommand(device, protocolRemove, protocolRemoveArgs);
	...
	private void executeOperations(Device device, String commandName, String arguments, Boolean immediate, String transactionId) {
		...
		Map<String, ProtocolObject> objects = profiles.getObjects().get(deviceName);
		...
		for (ResourceOperation operation: operations) {
			...
			ProtocolObject object = getProtocolObject(objects, objectName, transactionId);
	...
	private ProtocolObject getProtocolObject(Map<String, ProtocolObject> objects, String objectName, String transactionId) {
		ProtocolObject object = objects.get(objectName);
	...
	private String parseArguments(String arguments, ResourceOperation operation, Device device, ProtocolObject object, Map<String, ProtocolObject> objects) {
